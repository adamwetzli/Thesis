...
% ACTUALLY HOW DO WE POOL? DO WE REALLY AVERAGE OVER THE PAIRS OR JUST PLOT ALL PREDICTIONS FOR ALL PAIRS IN THE SAME PLOT?
...
The first observation is a genuine limitation. Every reported Brier score ($\frac{1}{N} \sum_{i=1}^{N} (p_i - o_i)^2$, where $p_i$ is M2 predicted probability and $o_i$ is the actual 
binary outcome $\{0, 1\}$) sits within a narrow band of $0.2441$-$0.2505$, where Brier of 0 means perfect predictions, 0.25 is a random guess, and 1 is always wrong.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%% GET THESE VALUES VALIDATED %%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\paragraph{GET THESE VALUES VALIDATED}

% PROMPT FOR CLAUDE: So you wrote in this passage: (paste it) about the improvements or performance of all models compared to the trivial baseline $\bar p(1-\bar p)$ 
% but you never gave the computation of these values, only the final result. Thus, i would like you to actually prove the computations fully, either in text or in a table so it is
% also fully understandable in the thesis. Also, are you sure you got the values for the computations from \ref{tab:nested_confusion_summary} and not the panel A of the confusion matrices?

Comparing each score against the Brier score of the trivial baseline that always predicts the population's own base rate, $\bar p(1-\bar p)$, using each model's own M1 base 
correctness rate from table \ref{tab:nested_confusion_summary}, shows that in fold 1 the raw M2 probability provides essentially no net predictive value over this naive baseline 
for five of six architectures as RandomForest, ExtraTrees, LSTM, and GRU all score marginally worse than the naive baseline ($+0.20$ to $+0.77\%$ relative), LightGBM ties it almost 
exactly, and only XGBoost manages a small improvement ($-0.20\%$). In the strict sense of a proper scoring rule averaged over the entire probability range, M2's fold 1 output is 
therefore not meaningfully more informative than simply guessing the unconditional base rate for the great majority of architectures tested.

Fold 2 tells a materially different story: every single model now beats its own naive baseline, by margins of roughly $0.85$-$1.42\%$ relative, with LightGBM showing the 
largest improvement and LSTM the smallest. This is modest in absolute terms, but it is a second, fully independent confirmation, via a proper scoring rule rather than via 
acceptance counts or coverage ratios, of the fold-over-fold improvement in M2's discriminative quality documented throughout this chapter.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%% GET THIS CITATION VALIDATED %%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\paragraph{GET THIS CITATION VALIDATED}
\textcite{shafer_tutorial_2008}'s central appeal is precisely that conformal validity does not require the underlying score to be a 
