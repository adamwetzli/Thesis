...
def run_nested_wfv(data: pd.DataFrame,
    ...
    for i, (o_train_idx, o_test_idx) in enumerate(outer_cv.split(data)):
        ...
        for i, m_name in enumerate(model_names):
            ...
            cal_probs = []
            if memory_window:
                for _, past_labs, past_probs in memory_window:
                    valid_mask = ~np.isnan(past_probs) # Filter out NaNs from the calibration set
                    cal_probs.extend(past_probs[valid_mask & (past_labs == 0)].tolist()) # label 0 -> M1 was incorrect (bad trade)
            
            # In the first fold there are no previous outer fold calibration sets so we use the current folds calibration set
            if len(cal_probs) == 0 and current_mistakes is not None:
                _, f_labs, f_probs = current_mistakes
                valid_mask = ~np.isnan(f_probs)
                cal_probs.extend(f_probs[valid_mask & (f_labs == 0)].tolist())
...
def run_wfv(data: pd.DataFrame, 
    ...
    }, f"./models/final_production/global_{winner_name}_prod.joblib")
